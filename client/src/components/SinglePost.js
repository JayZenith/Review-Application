...
function SinglePost() {
  ...
  useEffect(() => {
    axios
      .get(process.env.REACT_APP_HTTP_REQ+`/singlePost/byId3/${id}`)
      //.get(`http://3.20.232.190:3001/singlePost/byId3/${id}`)
      .then((response) => {
...
//thought is rerender any change to comments and abovce gets send back comment details
  ...
  useEffect(()=>{
    axios.get(process.env.REACT_APP_HTTP_REQ+`/comments3/${id}`).then((response) => {
    //axios.get(`http://3.20.232.190:3001/comments3/${id}`).then((response) => {
  ...
  const addComment = () => {
    axios
      .post(
        process.env.REACT_APP_HTTP_REQ+"/comments",
      //.post(
        //"http://3.20.232.190:3001/comments",
        {
  ...
  const deleteComment = (id) => {
    axios.delete(process.env.REACT_APP_HTTP_REQ+`/deleteComment/${id}`, {
    //axios.delete(`http://3.20.232.190:3001/deleteComment/${id}`, {
      headers: {
        accessToken: localStorage.getItem("accessToken"),
      },
    }).then(()=> {
                ...
                >
                    <div className={SinglePostCSS.singlePostAvatar}>
                      {postObject.ImageData? 
                        <img className={SinglePostCSS.imgAvatar} src={process.env.REACT_APP_HTTP_REQ+`/images/`+postObject.ImageData} width="150" height="80" alt="" />
                        //<img className={SinglePostCSS.imgAvatar} src={`http://3.20.232.190:3001/images/`+postObject.ImageData} width="150" height="80" alt="" />
                      ...
                      <div className={SinglePostCSS.commentFooter}>
                            <div className={SinglePostCSS.commentAvatarAndPic}>
                              <div className={SinglePostCSS.commentAvatar} onClick={()=> {
                                ...
                                {comment.ImageData?
                                //http://3.143.203.151:3001
                                  <img className={SinglePostCSS.imgAvatar} src={process.env.REACT_APP_HTTP_REQ+`/images/`+comment.ImageData} width="150" height="80" alt="" />
                                  //<img className={SinglePostCSS.imgAvatar} src={`http://3.20.232.190:3001/images/`+comment.ImageData} width="150" height="80" alt="" />
                                  :<></>
                                }
                              </div>
                              <div onClick={()=> {
